...
fn parse_feishu_resume_input(raw: &Value) -> Result<FeishuResumeInput> {
    serde_json::from_value::<FeishuResumeInput>(raw.clone())
        .context("invalid feishu-im effect input")
}

async fn append_feishu_resume_failure(
    log: &mut EventLog,
    activity_id: &str,
    attempt_id: &str,
    idempotency_key: &str,
...
fn build_feishu_transient_failure(
    activity_id: &str,
    attempt_id: &str,
    provider: &str,
    idempotency_key: &str,
...
fn parse_workflow_text_command(text: &str) -> Option<WorkflowTextCommand> {
...
async fn run_workflow_host_executor(
    state: &AppState,
    ctx: WorkflowDispatchRun<'_>,
    node: &beam_core::HostExecutorNode,
    // Already parsed by `prepare_host_executor`.
...
fn workflow_definition_search_dirs() -> Vec<PathBuf> {
...
async fn list_workflow_definitions() -> Result<Vec<WorkflowCatalogEntry>> {
    list_workflow_definitions_in(workflow_definition_search_dirs()).await
}

async fn list_workflow_definitions_in(dirs: Vec<PathBuf>) -> Result<Vec<WorkflowCatalogEntry>> {
...
async fn load_workflow_catalog_definition(
    workflow_id: &str,
...
async fn load_workflow_catalog_definition_in(
    workflow_id: &str,
    paths: Vec<PathBuf>,
) -> Result<Option<WorkflowCatalogDefinition>> {
    for path in paths {
...
fn load_workflow_definition_candidates(workflow_id: &str) -> Vec<PathBuf> {
    let mut candidates = vec![
        std::env::current_dir()
...
#[allow(dead_code)]
async fn build_workflow_run_summary(
    paths: &BeamPaths,
    run_id: &str,
...
async fn write_attempt_resume_sidecar(
    paths: &BeamPaths,
...
fn parse_attempt_resume_request_body(
    body: &[u8],
) -> Result<AttemptResumeRequest, (StatusCode, String)> {
    if body.is_empty() {
...
async fn wait_for_attempt_resume_ready(
    state: &AppState,
    key: &str,
    sidecar_path: &str,
...
fn feishu_outcome_json(outcome: &FeishuResumeOutcome) -> Value {
    serde_json::json!({
...
fn transient_failure_json(failure: &FeishuTransientFailure) -> Value {
    serde_json::json!({
...
fn resume_started_event_json(event: &beam_core::WorkflowEventEnvelope) -> Value {
    serde_json::json!({
...
fn build_workflow_resume_response(
    ...
    last_seq: u64,
    resume_started_event: Option<&beam_core::WorkflowEventEnvelope>,
    event_index: &HashMap<String, beam_core::WorkflowEventEnvelope>,
    snapshot: &beam_core::RunSnapshotDTO,
...
fn build_resume_reconcile_outcome(
    event_index: &HashMap<String, beam_core::WorkflowEventEnvelope>,
...
fn workflow_event_json(event: &beam_core::WorkflowEventEnvelope) -> Value {
    serde_json::json!({
        "eventId": event.event_id,
        "runId": event.run_id,
        "timestamp": event.timestamp,
...
fn resolve_activity_node_is_decision(
    workflow_def: &beam_core::WorkflowDefinition,
    activity: &beam_core::ActivityState,
) -> bool {
...
fn append_resume_wait_recovery(
    log: &mut EventLog,
    workflow_def: &beam_core::WorkflowDefinition,
    activity: &beam_core::ActivityState,
) -> Result<Option<Value>> {
...
fn append_resume_cancel_recovery(
    log: &mut EventLog,
    event_index: &HashMap<String, beam_core::WorkflowEventEnvelope>,
    activity: &beam_core::ActivityState,
) -> Result<Option<Value>> {
...
fn append_resume_worker_crashed(
    log: &mut EventLog,
    activity: &beam_core::ActivityState,
) -> Result<Option<Value>> {
    let latest = activity
