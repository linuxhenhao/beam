...
```
beam workflow run <workflowId> [key=value ...]
```

例如：

Lark 话题中（支持引号包裹含空格的值）：
```
/workflow run subagent-approval-feishu-send task="review and deploy PR #42"
/workflow run subagent-approval-feishu-send task='review the code'
```

CLI 中（支持 shell 引号传递含空格的值）：
```
beam workflow run subagent-approval-feishu-send task="review and deploy the PR #42"
```

通过 Dashboard 或 HTTP API 触发时，可以传递 typed JSON params（如 `{"enabled": true, "count": 42}`）。

**引号支持**：Lark `/workflow run` 采用 shell-like word parsing（通过 `shell-words` crate），支持单引号（`'`）、双引号（`"`）和反斜杠（`\`）转义。规则如下：

- **单引号**：单引号内所有字符保持字面意义，不支持转义。如 `task='review PR #42'`。
- **双引号**：双引号内支持反斜杠转义 `$`、`` ` ``、`"`、`\`、换行符。如 `task="say \"hello\""`。
- **反斜杠转义**：在无引号上下文中，反斜杠转义下一个字符。如 `path=a\ b` 得到 `a b`。
- **相邻引号拼接**：相邻的引号段和无引号段会自动拼接。如 `name="Hel"lo` 得到 `Hello`。这与标准 shell 行为一致。
- **未闭合引号**：引号未闭合时返回参数引号不匹配错误。
...
**文本入口类型转换**：CLI 和 Lark 文本入口中 `key=value` 的 value 总是以 string 形式传入。系统会根据 workflow params schema 的类型声明自动将 string 转换为目标 JSON 类型：

| schema type | 转换规则 | 示例 |
|------------|---------|------|
| `string` | 保持为字符串，不做转换 | `task=hello` → `"hello"` |
| `boolean` | 仅接受大小写敏感的 `true` / `false`（trim 后） | `verbose=true` → `true` |
| `number` | 按 JSON number 解析；接受 `1`, `1.5`, `-2`, `1e3`；拒绝 NaN/Inf/非数字 | `ratio=3.14` → `3.14` |
| `integer` | 仅接受十进制整数字符串如 `0`, `42`, `-1`；拒绝 `1.0`/`1.5`/`1e3` | `count=10` → `10` |
| `object` | 将 string 作为 JSON 解析，必须是 object；引号包裹以保护空格/符号 | `payload='{"a":1}'` → `{"a":1}` |
| `array` | 将 string 作为 JSON 解析，必须是 array | `tags='["a","b"]'` → `["a","b"]` |

**注意**：schema 中参数的 `default` 值必须是已正确类型的 JSON（如 `"default": false` 而不是 `"default": "false"`），系统不会对 default 值进行字符串类型转换。这样可以避免 schema 写错被掩盖。

### Workflow 参数（params）

Workflow 可声明 `params` schema 来定义运行时需要接受的参数：

```json
{
  "workflowId": "demo-params",
  "version": 1,
  "params": {
    "task": {
      "type": "string",
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `type` | string | 必需。参数类型：`string` / `number` / `integer` / `boolean` / `object` / `array` |
| `required` | bool | 可选。默认 `false`。`true` 时缺少该参数会拒绝 run |
| `default` | Value | 可选。未提供参数时使用的默认值，类型必须与 `type` 匹配 |
| `description` | string | 可选。参数说明，仅用于文档，不参与校验 |
| `format` | string | 可选。格式校验（仅适用于 `string` 类型）。支持：`date`、`date-time`、`email`。未知 format 会拒绝 run。 |

类型校验规则：

- `string`: JSON string
- `number`: JSON number
- `integer`: JSON number 且为整数（`42` 或 `42.0` 均可）
- `boolean`: JSON bool
- `object`: JSON object
- `array`: JSON array
- 未知 `type` 会返回错误（包含参数名和 type）

**format 校验规则**（仅适用于 string 类型参数）：

- `date`：必须是 `YYYY-MM-DD` 格式的真实日历日期（如 `2024-02-29` 通过，`2023-02-29` 拒绝）
- `date-time`：必须是合法的 RFC 3339 格式（如 `2026-06-17T12:34:56Z` 或 `2026-06-17T12:34:56+08:00`）
- `email`：必须恰好一个 `@`，local 和 domain 非空，domain 至少一个 `.`，domain 标签非空且不以 `-` 开头/结尾，不含空白字符
- 未知 format 会返回错误（包含参数名和 format 值）
- 若 format 用于非 string 类型参数，会返回错误

**常见错误信息**：

- 类型不匹配：`workflow parameter '<name>' expects type '<type>', got <actual>`
- 未知参数：`unknown workflow parameter(s): <names>. Available parameters: [<available>]`
- 未声明参数却传入参数（无 params 定义时）：`unknown workflow parameter(s): <names>. No parameters are declared for this workflow.`

**文本入口类型转换**：CLI 和 Lark 文本入口（`key=value` 形式）的所有 value 均作为 JSON string 传入。系统会根据 params schema 自动将 string 转换为目标类型（见上方快速开始的类型转换表）。类型转换失败的（如 boolean 传 `yes`、integer 传 `1.5`）会返回明确错误。schema default 值不做转换，必须是已正确类型的 JSON。

特殊规则：

- 若 workflow 未定义 `params`（或定义为空），**任何参数都会被拒绝**，错误提示 `No parameters are declared for this workflow`。
- 若 workflow 定义了 `params`，未在 schema 中声明的参数会被拒绝，错误包含 `unknown workflow parameter(s)`、未知名、可用参数名。
- `required` 的 string 参数空白/仅空白字符视为缺失，拒绝 run。
- 未传且非 required 无 default 的参数不会写入 params blob。
- 参数的 default 值会被写入 params blob（例如 `verbose` 没传但 `default: false`，则 blob 中会写入 `"verbose": false`）。
- 参数在 prompt 中可通过 `${params.name}` 引用。由于写入的是规范化的 typed JSON，`${params.enabled}` 返回 bool，`${params.count}` 返回 number，在字符串插值中按现有逻辑转为字符串。
