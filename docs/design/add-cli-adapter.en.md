...
- **Do not hand-roll boilerplate** — use the shared pieces in `crate::adapter`:
  ...
  - The submit-confirmation data source must match the file (and its format) the CLI really writes, or successful input gets reported as a failure. codex / traex share `codex.rs`, whose submit history is **append-only JSONL** (`~/.codex/history.jsonl` / `~/.trae/cli/history.jsonl`): `capture_history_boundary` takes a byte boundary and `read_recent_history_entries` reads only the delta. The file name **must end in `.jsonl`** — naming it `.json` takes the JSON-document path, and a single-entry JSONL file parses as one whole JSON value, so either way confirmation can never match. Note Traex splits history from rollouts (history under `~/.trae/cli/`, rollouts under `~/.traex/cli/`).
