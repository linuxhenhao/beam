...
(2026-06-27 更新) daemon 重启后尽量恢复运行状态，以下状态会持久化到 `state/` 目录：

### 已持久化的状态

| 状态 | 文件路径 | 说明 |
|------|----------|------|
| Sessions | `sessions/sessions.json` | 已有，session 本身已落盘 |
| Frozen cards | `sessions/frozen-cards/{session_id}.json` | 已有 |
| Pending response patches | `sessions/pending-response-patches/{session_id}.json` | 已有 |
| Workflow progress cards | `state/workflow-progress-cards.json` | 新增，run_id → card_message_id 映射 |
| Terminal ticket secret | `state/ticket-secret` | 已有 |
| Used terminal tickets | `state/used-tickets.json` | 新增，防重放 nonce 持久化 |
| Ask pending entries | `state/ask-pending.json` | 新增，TTL 30 分钟，oneshot 通道不跨重启 |
| Grant pending entries | `state/grant-pending.json` | 新增，TTL 10 分钟 |
| Pending session creates | `state/pending-creates.json` | 新增，TTL 30 分钟 |
| Webhook replay nonces | `state/replay-nonces.json` | 新增，定期 30 秒落盘 |
| Rate buckets | `state/rate-buckets.json` | 新增，定期 30 秒落盘 |
| Lark event dedupe | `state/recent-lark-events.json` | 新增，5 分钟窗口 |
| Final output retry queue | `state/final-output-retries.json` | 新增，重启后恢复未完成的 final output 投递 |
| Schedule tasks | `schedules.json` | 已有，新增 daemon scheduler loop 按 next_run_at 触发 |

### 已知限制

1. **Terminal Beam cookie → zellij cookie 映射不持久化**：出于安全考虑，Beam cookie jar 仅内存。重启后已登录的终端页面需要重新通过 ticket 获取新 Beam cookie，但 ticket secret 保持可验证。

2. **Ask pending 的 oneshot HTTP 等待通道不跨重启**：持久化的 ask 卡在重启后点击会显示"ask expired (daemon restarted)" toast，不会静默失败。

3. **Grant pending 状态**：已实现持久化和 TTL 清理，重启后未过期项仍可生效。

4. **Pending creates (目录选择)**：已实现持久化，重启后用户可在 TTL 内继续选择目录创建 session。

5. **Rate bucket 窗口精度**：定期 30 秒落盘可能导致极端情况下丢失最近 30 秒内的计数，窗口上限误差 ≤ 30 秒。不影响安全性。

6. **Scheduler loop**：daemon 启动后每 30 秒扫描 `schedules.json`，触发 `next_run_at` 到期的 enabled task。Once 类型执行后禁用，interval/cron 类型通过 `mark_run` 计算下一次。
