...
beam 此前对 codex 强制串行（忙时消息排队、等 idle 才逐条发），原因是 fallback 桥的 `CodexBridgeQueue` 单 `collecting` 指针归因没在 type-ahead 的 back-to-back user_message 排序下验证过。
...
- suppression window：t1 被丢后只 drain t2，t2 的窗口锚定 user2 的 dequeue 时间戳（既有 markTimeMs override），合并回复的 beam send 落在 t2 窗口内 → 正确抑制。
