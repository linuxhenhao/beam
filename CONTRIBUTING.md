...
```bash
git clone https://github.com/deepcoldy/beam.git
cd beam
pnpm install
pnpm build
...
AI Coding CLI (interactive TTY)
    ...
    |-- ~/.beam/bin/beam wrapper on PATH → `beam send/schedule/bots/thread` subcommands
    |
Lark API
...
src/
  ...
  skills/
    definitions.ts          # Built-in Skill markdown (beam-send/schedule/bots/thread-messages)
...
beam exposes its Lark-interaction surface as **CLI subcommands**
(`beam send`, `beam schedule`, `beam bots`,
`beam thread messages`) paired with auto-installed **Skills** that
...
2. Worker `PATH` is prepended with `~/.beam/bin`, which contains a
   `beam` shell wrapper written by the daemon at startup (points at the
   running daemon's `dist/cli.js` — always in sync).
3. `--append-system-prompt` flag injects the routing instruction
   ("user reads Lark, not terminal — use `beam send` for user-facing content")
   into each CLI session.
4. Every user message carries a per-message hint (`[回复请用 beam send]`)
...
|------------|-------------|
| `beam send [content]` | Send message to current thread (stdin / heredoc / `--content-file`; `--images` / `--files` / `--mention` flags) |
| `beam bots list` | List bots in current chat with their `open_id`s |
| `beam thread messages [--limit N]` | Fetch thread message history (JSON) |
| `beam schedule add <schedule> <prompt>` | Create scheduled task bound to current thread |
| `beam schedule list/remove/pause/resume/run` | Manage tasks |
...
(`{dataDir}/.beam-cli-pids/{pid}`). Works across every CLI that can
...
| `modelChoices` | Curated model candidates surfaced in `beam setup` (optional). Set when the CLI accepts a model flag (consumed in `buildArgs` via the `model` opt); omit when the CLI has no `--model` concept — setup then skips the model prompt for that CLI |
