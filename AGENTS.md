...
- Run the unified harness before pushing: `scripts/check-all.sh` (runs, in order: `check-rust-line-count.sh`, `check-clippy.sh`, `check-build.sh`, `check-tests.sh`). It is the CI entry point (`parity.yml`). Each check can also be run individually; narrower test runs use `cargo test -p <crate> <filter>`.
