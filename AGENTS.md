...
- Release flow (3 stages, automated via GitHub Actions):
  1. **Release PR** — On every push to `main`, `.github/workflows/release-pr.yml` runs `release-plz release-pr` to create or update a single Release PR containing version bumps, dependency updates, and the generated changelog. Nothing is published yet.
  2. **Git Tag + GitHub Release** — When the Release PR is merged, `.github/workflows/release-tag.yml` runs `release-plz release`, which creates a git tag and a GitHub Release with the changelog. On ordinary `main` pushes this workflow is a no-op. The `v*` tag push also triggers `.github/workflows/release.yml` to build the `beam-cli` binary and upload it as a release asset. **No crates.io publish occurs at this stage.**
