...
- Design-doc routing:
  ...
  - Adding a new CLI adapter: read `docs/design/add-cli-adapter.md`; English mirror: `docs/design/add-cli-adapter.en.md`.
...
- Run the unified harness before pushing: `scripts/check-all.sh` (runs, in order: `check-fmt.sh`, `check-rust-line-count.sh`, `check-clippy.sh`, `check-build.sh`, `check-tests.sh`). It is the CI entry point (`parity.yml`). Each check can also be run individually; narrower test runs use `cargo test -p <crate> <filter>`.
- Run clippy before pushing: `scripts/check-clippy.sh` (enforces zero warnings workspace-wide via `cargo clippy --workspace --all-targets -- -D warnings`). Clear any new warnings you introduced; keep the workspace at zero warnings.
...
- There is no repo `lint` or `format` script; do not assume one exists. `cargo clippy` and `cargo fmt` are standard toolchain checks you can run anytime; `scripts/check-all.sh` is the CI harness entry point that runs all checks.
...
- Release flow (3 stages, serialized via GitHub Actions):
  1. **Git Tag + GitHub Release** — On every push to `main` (including Release PR merges), `.github/workflows/release-tag.yml` runs `release-plz release`. If there are unpublished version bumps, it creates per-crate git tags and GitHub Releases with changelogs (`beam-core-vX.Y.Z`, `beam-daemon-vX.Y.Z`, `beam-worker-vX.Y.Z`, `beam-cli-vX.Y.Z`). On ordinary `main` pushes this workflow is a no-op. The `beam-cli-v*` tag is the workspace release sentinel: it triggers `.github/workflows/release.yml` to build the `beam-cli` binary and upload it as a release asset. **No crates.io publish occurs at this stage.**
  ...
  3. **crates.io Publish** — Any `beam-cli-v*` tag push or manual `workflow_dispatch` triggers `.github/workflows/publish.yml`. A `validate-tag` job checks that the tag is a **stable beam-cli semver tag** (`beam-cli-vX.Y.Z` only; no prerelease suffix); prerelease tags are skipped without triggering the production environment. Only stable tags proceed to the `publish` job, which is gated by the `production` GitHub Environment (can be configured with required reviewers). Crates are published in topological order: `beam-core` → `beam-daemon` → `beam-worker` → `beam-cli`, each with dry-run retry first, then publish retry (handles crates.io index delay).
  - **Prerelease tags** (e.g. `beam-cli-vX.Y.Z-beta.1`, `beam-cli-vX.Y.Z-canary.1`) trigger the binary build & GitHub Release upload, but are **NOT** published to crates.io (crates.io has no dist-tag / non-latest concept).
...
- Local api token: the daemon generates `$BEAM_HOME/api-token` (0600) and rotates it daily (token embeds its issue timestamp — file mtime is not trusted; previous token accepted for 1 minute grace; see `crates/beam-daemon/src/api_token.rs` and `crates/beam-core/src/api_token.rs`). beam-cli never sends the token on the wire: `api_client` HMAC-signs each request (`x-beam-ts` / `x-beam-nonce` / `x-beam-sig` headers, signature covers method + path/query + body hash); `dashboard_gate` verifies the signature (±60s window, nonce replay protection) or accepts a dashboard token. Keep routes called by beam-cli behind `dashboard_gate`; do not move them into `open_routes` (unauthenticated).
